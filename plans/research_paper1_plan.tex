...
%----------------------------------------------------------------------
\begin{enumerate}
    \item Offline baselines (no live VLA needed):
          \texttt{python3 -m src.step3\_control\_baselines --all --episode 0}
    \item Live UnifoLM dual-process (omit \texttt{--mock}):
          \texttt{--bridge esn}, then \texttt{zoh}, then \texttt{linear}
          (\texttt{python3 -m src.step3\_dual\_thread\_mujoco --bridge esn --duration\_s 10}).
    \item Or one-shot: \texttt{python3 -m src.step3\_sim\_comparison --episode 0 --duration\_s 10}
    \item Next week: same bridges on Jetson AGX Thor $\to$ G1 via \texttt{s2r} (do not block sim on hardware).
\end{enumerate}
