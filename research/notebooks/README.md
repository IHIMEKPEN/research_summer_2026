...
2. Open `step3_dual_thread_mujoco.ipynb` → `MOCK=False` → Run All (timing: esn → zoh → linear → pid).
3. Open `step3_live_wipe_success.ipynb` → `MOCK=False` → Run All (live wipe grasp/contact/task).
4. Or open `step3_sim_comparison.ipynb` → `MOCK=False` → Run All (offline + live suite).
