...
_hf = {'HF_HOME': '/raid/data/aihimekpen/hf_cache', 'HUGGINGFACE_HUB_CACHE': '/raid/data/aihimekpen/hf_cache/hub'}
for k, v in _hf.items():
    os.environ[k] = v

print("nsys child python:", sys.executable, flush=True)
import torch
...
print("Loading UnifoLM (may download weights)…", flush=True)
...
print("Model ready; mock=", model.model is None, flush=True)
...
for i in range(PROFILE_STEPS):
    print(f"profile {i+1}/{PROFILE_STEPS}", flush=True)
...
print("NSYS target run complete", flush=True)
